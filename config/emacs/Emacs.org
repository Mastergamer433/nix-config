...
#+begin_src emacs-lisp
  ...
  (defun keo/wm-open-config (config)
    (select-frame (make-frame '((name . "Emacs-Config"))))
    (find-file (car (cdr (assoc config keo/configs))))
    (let ((line (cl-first (cddr (assoc config keo/configs))))
          (regex (cl-first (cdddr (assoc config keo/configs)))))
      (if line
          (goto-line line))
      (if regex
          (progn
