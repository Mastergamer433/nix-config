...
#+begin_src emacs-lisp
  (defun keo/string-trim-final-newline (string)
...
#+begin_src emacs-lisp
  (defun keo/wm-get-title-of-all-clients ()
    (let ((titles (list))
      ...
      (mapcar 
       (lambda (c)
         (push (keo/string-trim-final-newline (keo/wm-get-title-of-client c)) titles))
       clients)
      titles))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun keo/wm-open-config (config)
    (select-frame
     (make-frame
      '((name . "Emacs-Config"))))
    (find-file
     (car (cdr (assoc config keo/configs))))
    (let ((line (cl-first
                 (cddr
  ...
  (keo/wm-open-config "emacs")

  ;; (keo/wm-open-config "herbstluft")
#+end_src
...
#+begin_src emacs-lisp
  ...
  (cl-defun proxmox-display-vms (data)
    (let ((i 0)
          (len (length (assoc-default 'data data)))
          (body (assoc-default 'data data))
          (buffer (get-buffer-create "*Proxmox-VMs*")))
      (with-current-buffer buffer
        (erase-buffer)
        (while (< i len)
          (insert "Name: " (assoc-default 'name (aref body i)))
          (insert " Cpu Usage: " (format "%s" (* 1000 (assoc-default 'cpu (aref body i)))))
          (insert "\n")
          (setq i (1+ i)))
  ...
  (defun proxmox-select-vm (vms)
    (let ((names '())
          (vm "")
          (res '())
          (i 0)
          (body (assoc-default 'data vms))
          (len (length (assoc-default 'data vms))))
      (while (< i len)
        (setq names (append names (list (assoc-default 'name (aref body i)))))
        (setq i (1+ i)))
      (setq vm (completing-read
                "Select VM"
                names
                nil
                t))
      (setq i 0)
      (while (< i len)
        (if (string= vm (format "%s" (assoc-default 'name (aref body i))))
            (setq res (aref body i)))
        (setq i (1+ i)))
      res))

  (defun proxmox-clone-vm (vm-id new-id new-name)
    (request (format "%s/node/pve/qemu/%s/clone" proxmox-base-url vm-id)
      :type "POST"
      :data `(("newid" . ,new-id) ("node" . "pve") ("vmid" . ,vm-id) ("name" . ,new-name) ("full" . "true"))
      :headers `(("Authorization" . ,(format "PVEAPIToken=%s" (password-store-get "proxmox/api"))))
      :parser 'json-read
      :success (cl-function
                (lambda (&key data &allow-other-keys)
                  (message "Success")))))
  (defun proxmox-next-vm-id (vms)
    (let ((i 0)
          (body (assoc-default 'data vms))
          (len (length (assoc-default 'data vms)))
          (id 0))
      (while (< i len)
        (cond ((and (< id (assoc-default 'vmid (aref body i)))
                    (not (= id 1000)))
               (setq id (assoc-default 'vmid (aref body i)))))
        (setq i (1+ i)))
      id))
  (cl-defun proxmox-clone ()
    (interactive)
    (let ((vm (proxmox-select-vm (proxmox-get-vms))))
      (proxmox-clone-vm (assoc-default 'vmid vm) (proxmox-next-vm-id (proxmox-get-vms)) (completing-read "New vm name: " (list)))))
  ;; (((body (assoc-default 'data data))))

  (proxmox-display-vms (proxmox-get-vms))
  (message "%s" (proxmox-get-vms))
  (proxmox-get-vms)

  (message "%s" (proxmox-select-vm (proxmox-get-vms)))
#+end_src
...
** Awesomewm denote new note
#+begin_src emacs-lisp
  (defun keo/awesomewm-denote-new-note ()
